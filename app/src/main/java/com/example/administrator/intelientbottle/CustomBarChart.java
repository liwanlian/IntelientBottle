...
public class CustomBarChart extends View {
    ...
    public void init() {
        ...
        paintCoordinate.setStyle(Paint.Style.STROKE);//描边
        ...
        paintRectF.setStyle(Paint.Style.FILL);//填充内部
        ...
        paintValue.setAntiAlias(true);//设置抗锯齿
        paintValue.setDither(true);//设置是否抖动
    ...
    private void drawAxesLine(Canvas canvas, Paint paint) {
        // X
        canvas.drawLine(xPoint, yPoint, this.getWidth() - margin / 6, yPoint, paint);//左上和右下的坐标
    ...
    private void drawCoordinate(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i <= (xLabel.length - 1); i++) {
            ...
            canvas.drawText(xLabel[i], startX-100, this.getHeight() - margin / 6, paint);
        }
    ...
    private void drawBar(Canvas canvas, Paint paint, int data[], List<Integer> colorList) {
        for (int i = 1; i <= (xLabel.length - 1); i++) {
            int startX = xPoint + i * xScale;
            RectF rect = new RectF(startX-150, toY(data[i - 1]), startX -50, this.getHeight() - margin - 2);//RectF 矩形类
            if (i % 2 == 1) {
    ...
    private void drawBars(Canvas canvas, Paint paint, List<int[]> dataList, List<Integer> colorList) {
        for (int i = 1; i <= (xLabel.length - 1); i++) {
            int startX = xPoint + i * xScale;
            paint.setColor(ContextCompat.getColor(getContext(), colorList.get(0)));
            RectF rect1 = new RectF(startX - 170, toY(dataList.get(0)[i - 1]), startX - 110,
                    this.getHeight() - margin - 2);
            canvas.drawRect(rect1, paint);

            paint.setColor(ContextCompat.getColor(getContext(), colorList.get(1)));
            RectF rect2 = new RectF(startX - 105, toY(dataList.get(1)[i - 1]), startX -50,
                    this.getHeight() - margin - 2);
    ...
    private void drawValue(Canvas canvas, Paint paint, int data[], int color) {
        ...
        for (int i = 1; i <= (xLabel.length - 1); i++) {
            canvas.drawText(data[i - 1] + "ml", xPoint + i * xScale-100, toY(data[i - 1]) - 5, paintValue);
        }
    }
    ...
    private void drawValues(Canvas canvas, Paint paint, List<int[]> dataList, int color) {
        ...
        for (int i = 1; i <= (xLabel.length - 1); i++) {
            ...
            canvas.drawText(dataList.get(0)[i - 1] + "ml", startX - 140,
                    toY(dataList.get(0)[i - 1]) - offsetY1, paintValue);
            canvas.drawText(dataList.get(1)[i - 1] + "ml", startX -75,
